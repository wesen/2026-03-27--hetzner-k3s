...
- [ ] Add the Vault policy and Kubernetes auth role for the snapshot job
- [ ] Add replayable ticket scripts for seeding any Vault-side state needed by the snapshot job
- [ ] Add the GitOps manifests for the Vault snapshot job runtime surface
- [ ] Add a `CronJob` that authenticates through Vault Kubernetes auth, creates a Raft snapshot, and uploads it to object storage
- [ ] Ensure the job fails closed if authentication or snapshot creation fails

## Phase 6: Validation

- [ ] Run the Vault snapshot job manually once
- [ ] Verify that a `vault/` artifact lands in Hetzner Object Storage
- [ ] Inspect the snapshot artifact size and naming shape
- [ ] Re-run `docmgr doctor`

## Phase 7: Restore and operator docs

- [ ] Write the operator playbook for Vault snapshot restore and Hetzner server-backup usage
- [ ] Decide whether a safe Vault scratch restore drill is feasible in this environment or should stay documented-only for now
- [ ] Update the main repo docs if the new backup posture should be visible outside the ticket

## Phase 8: Closeout

- [ ] Update the diary, changelog, and ticket index with the final live state
