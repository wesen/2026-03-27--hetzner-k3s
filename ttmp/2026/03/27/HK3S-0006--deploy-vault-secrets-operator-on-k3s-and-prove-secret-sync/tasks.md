...
- [ ] Confirm VSO is the preferred first secret-consumption path over Vault Agent injection for this cluster
- [ ] Confirm the namespace and service account model for the first sync test
- [ ] Confirm the CRD layout and Argo CD packaging strategy

## Phase 2: Repo-managed deployment scaffold

- [ ] Add the Argo CD `Application` for Vault Secrets Operator
- [ ] Add the first `VaultConnection` and `VaultAuth` definitions
- [ ] Add the smoke `VaultStaticSecret` and destination Kubernetes `Secret` contract
- [ ] Document the operator flow and validation steps
